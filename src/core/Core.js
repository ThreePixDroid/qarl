...
export class Core extends EventEmitter {
	...
	onBegin(...args) {
		super.on(EVENTS.BEGIN, ...args);
		return this;
	}

	onceBegin(...args) {
		super.once(EVENTS.BEGIN, ...args);
		return this;
	}
	...
	/**
		* Resets timeline state without recalculating dynamic properties.
		* @private
		*/
	_resetState() {
	...
	/**
	 * Resets timeline state and recalculates dynamic properties.
	 * @private
	 */
	_processState() {
		this._resetState();
	 ...
	 */
	_processEasing() {
		this._rawEasing = typeof this.settings.easing === 'string'
			? easings[this.settings.easing] || easings.linear
			: this.settings.easing;

		this._easing = this.reversed ? this._reversedEasing : this._rawEasing;
